...
def image_cut(input,output):
    ...
    path_out = output
    img = cv2.imread(path)
    img = np.pad(img,pad_width=((64,64),(64,64)),mode="edge")
    img_shape = img.shape
    ...
    for h in range(h_step):
        for w in range(w_step):
            image_sample = img[(h * 128):(h * 128 + 256 ),
                           (w * 128):(w * 128 + 256), :]
            image_list.append(image_sample)
        image_list.append(img[(h * 128):(h * 128 + 256), -128:, :])
    for w in range(w_step - 1):
        image_list.append(img[-128:, (w * 128):(w * 128 + 256), :])
    image_list.append(img[-128:, -128:, :])

...
def image_cut(input,output):
    ...
    #print(h_step, w_step)
    h_rest = -(img.shape[0] - 256 * h_step)
    w_rest = -(img.shape[1] - 256 * w_step)
    #print(h_rest, w_rest)
    image_list = []
    for h in range(h_step):
        for w in range(w_step):
            image_sample = img[(h * 256):(h * 256 + 256),
                           (w * 256):(w * 256 + 256), :]
            image_list.append(image_sample)
        image_list.append(img[(h * 256):(h * 256 + 256), -256:, :])
    for w in range(w_step - 1):
        image_list.append(img[-256:, (w * 256):(w * 256 + 256), :])
    image_list.append(img[-256:, -256:, :])
    for i in range(len(image_list)):
        cv2.imwrite(path_out + '/' + str(i) + '.png', image_list[i])
        data_list.append(str(i) + '.png')
    print("cut images saved in :" + path_out)
    return h_step,w_step,h_rest,w_rest,img_shape,data_list



def image_comb(h_step,w_step,h_rest,w_rest,img_shape,outname,inpath):
    path = inpath
    ori_image = img_shape
    predict_list = []
...
def main():
    ...
    h_step,w_step,h_rest,w_rest,img_shape,data_list = image_cut(in_path,cut_path)
    check_gpu(args.use_gpu)
    print('predicting......')
    infer(args,cut_path,data_list,comb_path)
    print('combining......')
    image_comb(h_step,w_step,h_rest,w_rest,img_shape,outname,comb_path)
